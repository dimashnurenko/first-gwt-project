...
public class EditEmployeeCallBackTest {
    ...
    private CallBack editEmplyeeCallback;

    @Before
    public void setUp() {

        this.editEmplyeeCallback = new CallBack() {
            @Override
            public void onChangeTableOfEmployee(Employee empl) {

                employees.remove((Employee)anyObject());
    ...
    public void testEditEmployeeCallBackBehavior() {
        ...
        editEmplyeeCallback.onChangeTableOfEmployee(employee);

        InOrder order = inOrder(employees, view);

        order.verify(employees).remove((Employee)anyObject());
